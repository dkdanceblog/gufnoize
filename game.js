...
function stageScreen() {
  ...
  drawPixelText("ТАПНИ ПО КАРТОЧКЕ ИЛИ НАЖМИ 1 / 2 / 3", W / 2, 506, 24, "#fff", "center");
  drawPixelText("ВЫБОР СРАЗУ НАЧИНАЕТ БОЙ", W / 2, 548, 28, "#fff6b0", "center");

  ctx.fillStyle = "rgba(0,0,0,0.48)";
  ctx.fillRect(W / 2 - 360, 606, 720, 62);
  ctx.strokeStyle = "rgba(255,246,176,0.42)";
  ctx.lineWidth = 3;
  ctx.strokeRect(W / 2 - 360, 606, 720, 62);

  drawPixelText("УПРАВЛЕНИЕ: ← → ДВИЖЕНИЕ   ↑/SPACE ПРЫЖОК   ↓ ПРИСЕСТЬ", W / 2, 616, 19, "#fff", "center");
  drawPixelText("АТАКИ: A — УДАР   S — СИЛЬНЫЙ   D — СПЕЦ", W / 2, 642, 19, "#fff6b0", "center");
}
...
function endScreen() {
  ...
  drawPixelText("НАЖМИ ЛЮБУЮ КНОПКУ ИЛИ КОСНИСЬ ЭКРАНА", W / 2, 575, 24, "#fff", "center");
  drawPixelText("ВЕРНУТЬСЯ К ВЫБОРУ ПЕРСОНАЖА", W / 2, 622, 24, "#ddd", "center");
}

function goToCharacterSelectAfterMatch() {
  stopBattleMusic();
  releaseAllControls();
...
document.querySelectorAll("#touch-controls button").forEach((btn) => {
  ...
  const up = (e) => {
    e.preventDefault();
    if (GAME.state !== "end") pressAction(action, false);
  };
...
if (touchControlsRoot) {
  touchControlsRoot.addEventListener("contextmenu", (e) => e.preventDefault());
  touchControlsRoot.addEventListener("selectstart", (e) => e.preventDefault());
}
